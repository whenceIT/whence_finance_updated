        ...
        <div class="box-body table-responsive">
            <table class="table  table-bordered table-hover table-striped" id="data-table">
                ...
                @foreach($data as $key)
                    ...
                    <tr>
                    @php
    $accountNo = $key->client->account_no ?? 'UNKNOWN-UNKNOWN-000-000000';

    // Compute loan cycle
    $cycle = \App\Models\Loan::where('client_id', $key->client_id)
        ->where('loan_product_id', $key->loan_product_id)
        ->where('status', 'disbursed')
        ->where('id', '<=', $key->id)
        ->count();

    // Pad cycle to 3 digits
    $cyclePadded = str_pad($cycle, 3, '0', STR_PAD_LEFT);

    // Replace the static '-000-' with the dynamic cycle
    $formattedAccount = str_replace('-000-', '-' . $cyclePadded . '-', $accountNo);
@endphp



                        <td><a href="{{ url('loan/'.$key->id.'/show') }}" data-toggle="tooltip" title="Click to view">{{ $formattedAccount }}</a></td>
                        <td>
                            @if(!empty($key->office))
                                {{$key->office->name}}
                            @endif
