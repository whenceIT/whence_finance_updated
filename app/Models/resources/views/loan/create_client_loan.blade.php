...
<style>
    ...
    <div class="box box-primary">
       ...
       value="{{ \Carbon\Carbon::parse(date('Y-m-d'))->format('d-m-Y') }}" required>
...
<input type="hidden" name="created_date" id="created_date" value="{{ date('Y-m-d') }}">
                ...
                <div class="form-group">
                    ...
                    <option value="months" {{ old('loan_term_type', 'months') == 'months' ? 'selected' : '' }}>{{trans_choice('general.month',2)}}</option>
<option value="days" {{ old('loan_term_type') == 'days' ? 'selected' : '' }}>{{trans_choice('general.day',2)}}</option>
<option value="weeks" {{ old('loan_term_type') == 'weeks' ? 'selected' : '' }}>{{trans_choice('general.week',2)}}</option>
<option value="years" {{ old('loan_term_type') == 'years' ? 'selected' : '' }}>{{trans_choice('general.year',2)}}</option>
    ...
    <input type="number" name="interest_rate" class="form-control"
       ...
       step="0.01"
       value="{{ old('interest_rate', $loan_product->default_interest_rate) }}"
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <input type="text" name="expected_disbursement_date" class="form-control date-picker"
                               placeholder=""
                               id="expected_disbursement_date"  value="{{date("Y-m-d")}}" >
                    </div>
                    ...
                    <div class="col-md-3">
                        <input type="text" name="expected_first_repayment_date" class="form-control date-picker"
                               placeholder="" 
                               id="expected_first_repayment_date" value="{{date("Y-m-d")}}">
                    </div>
    ...
    <tbody id="charges_table">
        @foreach (\App\Models\Charge::all() as $key)
            @if (!empty($key->charge))
                @php
                    $charge = $key->charge;
                    $computed_amount = $charge->charge_option === "flat"
                        ? $key->amount
                ...
                <tr id="row{{ $charge->id }}" data-charge-type="{{ $charge->charge_type }}">
                    ...
                    <!-- Hidden Charge ID for submission -->
                    ...
                    <td>
                        @if ($charge->override == 1)
                            <input type="number" step="0.01"
                                   ...
                                   value="{{ number_format($computed_amount, 2, '.', '') }}" required>
                        @else
                    ...
                    <!-- Hidden Date Column (optional future use) -->
                    <td style="display: none;">
                        @if ($charge->charge_type === 'specified_due_date')
                            <input type="date" name="charge_date[{{ $charge->id }}]" class="form-control">
                        @else
