...
@section('content')
    ...
    @if($client->client_type === 'individual')
        <li class="list-group-item">
            ...
            <a class="pull-right">{{ trans("general." . $client->gender) }}</a>
        </li>

        <li class="list-group-item">
            <b>{{trans_choice('general.dob',1)}}</b>
            <a class="pull-right">{{ $client->dob ? \Carbon\Carbon::parse($client->dob)->format('d/m/Y') : '' }}
            </a>
        </li>

        <li class="list-group-item">
            <b>{{trans_choice('general.marital_status',1)}}</b>
            <a class="pull-right">{{ trans("general." . $client->marital_status) }}</a>
        </li>
        ...
        <li class="list-group-item">
            ...
            <a class="pull-right">{{ number_format($client->salary, 2) }}</a>
        </li>
    ...
    @elseif($client->client_type === 'business')
        ...
        <li class="list-group-item">
            ...
            <a class="pull-right">{{ $client->key_contact_person }} </a>
        </li>
        ...
        <li class="list-group-item">
            <b>{{trans_choice('general.monthly_turnover',1)}}</b>
            <a class="pull-right">{{ number_format($client->monthly_turnover, 2) }}</a>
        </li>
    @endif

    {{-- Shared Fields --}}
    <li class="list-group-item"><b>{{trans_choice('general.mobile',1)}}</b><a class="pull-right">{{ $client->mobile }}</a></li>
    <li class="list-group-item"><b>{{trans_choice('general.phone',1)}}</b><a class="pull-right">{{ $client->phone }}</a></li>
    <li class="list-group-item"><b>{{trans_choice('general.email',1)}}</b><a class="pull-right">{{ $client->email }}</a></li>
    <li class="list-group-item">
        <b>{{trans_choice('general.registration',1)}} {{trans_choice('general.date',1)}}</b>
        <a class="pull-right">{{ $client->joined_date ? \Carbon\Carbon::parse($client->joined_date)->format('d/m/Y') : 'N/A' }}
        </a>
    </li>
    <li class="list-group-item">
        <b>{{trans_choice('general.status',1)}}</b>
        <a class="pull-right">{{ trans("general." . $client->status) }}</a>
    </li>
    <li class="list-group-item">
        <b>{{trans_choice('general.blacklisted',1)}}</b>
        <a class="pull-right">{{ $client->blacklisted ? trans('general.yes') : trans('general.no') }}</a>
    </li>
        ...
        <li class="list-group-item">
            ...
            <a class="pull-right">
                @if($key->custom_field->field_type == 'checkbox')
                    @foreach(unserialize($key->name) as $v => $val)
                        {{ $val }}<br>
                    @endforeach
                @else
                    {{ $key->name }}
            ...
            <div class="modal-content">
                ...
                <form method="post" action="{{url('client/'.$client->id.'/inactive')}}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
    <label for="inactive_reason" class="control-label col-md-3">
        {{ trans_choice('general.reason', 1) }}
    </label>
    <div class="col-md-9">
        <select name="inactive_reason" id="inactive_reason" class="form-control select2" required>
            <option value="">-- Select Reason --</option>
            @foreach(\App\Models\BlacklistReason::all() as $reason)
                <option value="{{ $reason->name }}" {{ old('inactive_reason') == $reason->name ? 'selected' : '' }}>
                    {{ $reason->name }}
                </option>
            @endforeach
        </select>
        ...
        <div class="modal-content">
            ...
            <form method="post" action="{{ url('client/' . $client->id . '/transfer') }}"
                ...
                <div class="modal-body">
                    ...
                    {{-- Transfer Type --}}
                    <div class="form-group">
                        <label for="transfer_type" class="control-label col-md-3">Transfer To</label>
                        <div class="col-md-9">
                            <select name="transfer_type" class="form-control" id="transfer_type" required>
                                ...
                                <option value="both">Branch and Staff</option>
                            </select>
                        </div>
                    </div>

                    {{-- Branch Selection --}}
                    <div class="form-group transfer-option branch-option hidden">
                        <label for="office_id" class="control-label col-md-3">{{ trans_choice('general.branch',1) }}</label>
                        <div class="col-md-9">
                            <select name="office_id" class="form-control select2" id="office_id">
                                <option></option>
                                @foreach(\App\Models\Office::where('id', '!=', $client->office_id)->get() as $key)
                                    <option value="{{ $key->id }}">{{ $key->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    {{-- Staff Selection --}}
                    <div class="form-group transfer-option staff-option hidden">
                        <label for="staff_id" class="control-label col-md-3">Staff Member</label>
                        <div class="col-md-9">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal">{{ trans_choice('general.close',1) }}</button>
                    <button type="submit" class="btn btn-primary">{{ trans_choice('general.save',1) }}</button>
                </div>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const branchOption = document.querySelector('.branch-option');
    const staffOption = document.querySelector('.staff-option');

    function toggleTransferOptions() {
        const val = transferType.value;

        branchOption.classList.add('hidden');
        staffOption.classList.add('hidden');

        if (val === 'branch') {
            branchOption.classList.remove('hidden');
        } else if (val === 'staff') {
            staffOption.classList.remove('hidden');
        } else if (val === 'both') {
            branchOption.classList.remove('hidden');
            staffOption.classList.remove('hidden');
        }
    ...
    transferType.addEventListener('change', toggleTransferOptions);
    toggleTransferOptions(); // initialize on page load
});
</script>
