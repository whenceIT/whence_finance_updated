...
@section('content')
    ...
    <div class="col-md-3">
        <select name="loan_product_id" class="form-control select2" id="loan_product_id" required>
            <option value="0" @if(($loan_product_id ?? "0")=="0") selected @endif>
                {{trans_choice('general.all',1)}}
            </option>
            @foreach(\App\Models\LoanProduct::all() as $p)
                <option value="{{$p->id}}"
                        @if(($loan_product_id ?? "0")==$p->id) selected @endif>
                    {{$p->name}}
                </option>
            @endforeach
        </select>
    ...
    <li>
        <a href="{{url('report/loan_report/age_analysis/pdf?end_date='.$end_date.'&office_id='.$office_id.'&loan_officer_id='.$loan_officer_id.'&loan_product_id='.$loan_product_id)}}"
           target="_blank"><i class="icon-file-pdf"></i>
            {{trans_choice('general.download',1)}} {{trans_choice('general.to',1)}} {{trans_choice('general.pdf',1)}}
        </a>
    </li>

    <li>
        <a href="{{url('report/loan_report/age_analysis_principle/excel?end_date='.$end_date.'&office_id='.$office_id.'&loan_officer_id='.$loan_officer_id.'&loan_product_id='.$loan_product_id)}}"
           target="_blank"><i class="icon-file-excel"></i>
            {{trans_choice('general.download',1)}} {{trans_choice('general.to',1)}} {{trans_choice('general.excel',1)}}
        </a>
    </li>

    <li>
        <a href="{{url('report/loan_report/age_analysis/csv?end_date='.$end_date.'&office_id='.$office_id.'&loan_officer_id='.$loan_officer_id.'&loan_product_id='.$loan_product_id)}}"
           target="_blank"><i class="icon-download"></i>
            {{trans_choice('general.download',1)}} {{trans_choice('general.to',1)}} {{trans_choice('general.csv',1)}}
        </a>
    </li>
</ul>
                ...
                <table class="table table-condensed table-hover " style="font-size: 12px">
                    ...
                    @foreach($data as $key)
                        ...
                        if ($amount_in_arrears > 0) {
                            ...
                            if ($key->loan_product->allow_custom_grace == 1 && strtotime($key->expected_maturity_date) >= $end_date_stamp) {
                                $on_time_balance += $outstanding_principal;
                            }
                        ...
                        @if($days_in_arrears >= 0)
                            <tr>
                                ...
                                <td>
                                {{$key->client->bank_account_number}}
                                </td>
                                <td>
