...
if (!empty($start_date)) {
    ...
    /* -----------------------------
       BASE REPAYMENT QUERY
    ----------------------------- */
    $repaymentBase = LoanTransaction::where('transaction_type', 'repayment')
        ->where('reversed', 0)
    ...
    $data = (clone $repaymentBase)
        ->where('payment_apply_to', 'full_payment')
        ->get();

    $part_data = (clone $repaymentBase)
        ->where('payment_apply_to', 'part_payment')
        ->get();

    $reloans_data = (clone $repaymentBase)
