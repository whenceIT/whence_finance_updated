...
@section('content')
    <div class="box box-primary">
        ...
        <form method="post" action="{{url('expense/'.$expense->id.'/update')}}" class="form-horizontal" enctype="multipart/form-data">
            {{csrf_field()}}
            <div class="box-body">
                    ...
                    <div class="col-md-3">
                        <select name="office_id" class="form-control select2" id="office_id" required>
                            ...
                            @foreach(\App\Models\Office::all() as $key)
                                <option value="{{$key->id}}" @if($expense->office_id==$key->id) selected @endif>{{$key->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <select name="recurring" class="form-control select2" id="recurring"
                                required>
                            <option value="0" @if($expense->recurring==0) selected @endif>{{trans_choice('general.no',1)}}</option>
                            <option value="1" @if($expense->recurring==1) selected @endif>{{trans_choice('general.yes',1)}}</option>
                        </select>
                    </div>
                </div>
                <div id="recur">
                    <div class="form-group">
                        ...
                        <div class="col-md-3">
                            <select name="recur_type" class="form-control select2" id="recur_type">
                                <option value="day" @if($expense->recur_type=="day") selected @endif>{{trans_choice('general.day',1)}}</option>
                                <option value="week" @if($expense->recur_type=="week") selected @endif>{{trans_choice('general.week',1)}}</option>
                                <option value="month" @if($expense->recur_type=="month") selected @endif>{{trans_choice('general.month',1)}}</option>
                                <option value="year" @if($expense->recur_type=="year") selected @endif>{{trans_choice('general.year',1)}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-3">
                            <input type="text" name="recur_start_date" class="form-control date-picker"
                                   value="{{$expense->recur_start_date}}"
                                   id="recur_start_date">
                        </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-8">
                        <textarea name="notes" class="form-control "
                                  placeholder=""
                                  id="notes" rows="3">{{$expense->notes}}</textarea>
                    </div>
                </div>
                            ...
                            <div class="col-md-8">
                                ...
                                @if($key->field_type=="select")
                                    <select class="form-control touchspin" name="custom_field_{{$key->id}}"
                                        ...
                                        @foreach(explode(',',$key->select_values) as $v)
                                            @if(!empty(\App\Models\CustomFieldMeta::where('custom_field_id',$key->id)->where('parent_id',$expense->id)->where('category','expenses')->first()))
                                                @if(\App\Models\CustomFieldMeta::where('custom_field_id',$key->id)->where('parent_id',$expense->id)->where('category','expenses')->first()->name==$v)
                                ...
                                @if($key->field_type=="radiobox")
                                    @foreach(explode(',',$key->radio_box_values) as $v)
                                        <div class="radio">
                                            <label>
                                                @if(!empty(\App\Models\CustomFieldMeta::where('custom_field_id',$key->id)->where('parent_id',$expense->id)->where('category','expense')->first()))
                                                    @if(\App\Models\CustomFieldMeta::where('custom_field_id',$key->id)->where('parent_id',$expense->id)->where('category','expense')->first()->name==$v)
                                ...
                                @if($key->field_type=="checkbox")
                                    @if(!empty(\App\Models\CustomFieldMeta::where('custom_field_id',$key->id)->where('parent_id',$expense->id)->where('category','expense')->first()))
                                        <?php $c = unserialize(\App\Models\CustomFieldMeta::where('custom_field_id',
                                            $key->id)->where('parent_id', $expense->id)->where('category',
                                            'expense')->first()->name); ?>
