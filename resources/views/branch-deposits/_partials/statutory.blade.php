@php
    ...
    $deposits = \App\Models\Deposit::join('bank_deposit_log', 'deposits.id', '=', 'bank_deposit_log.deposit_id')
        ...
        ->where('bank_deposit_log.deposit_type', 5)
    ...
    $depositType = \App\Models\DepositType::find(5);

    
    $depositName = $depositType->name ?? 'Administration Department fee deposit';
    $monthlyRequired = $depositType->monthly_amount ?? 8000;
...
$(document).ready(function() {
    ...
    var currentDepositType = 5; // Statutory payments
    ...
    $(document).on('change', '.payment-method', function () {
        ...
        switch ($(this).val()) {
            case 'airtel': hint.text('Format: MP260223.0953.J76581'); referenceInput.attr('placeholder', 'MP260223.0953.J76581'); break;
            case 'airtel_app': hint.text('Format: APCZM194947529952000'); referenceInput.attr('placeholder', 'APCZM194947529952000'); break;
            case 'zanaco_express': hint.text('Format: 12 digit number (002504072516)'); referenceInput.attr('placeholder', '002504072516'); break;
            case 'mtn': hint.text('Format: 10 digit number (8704564481)'); referenceInput.attr('placeholder', '8704564481'); break;
            case 'zanaco_cash': hint.text('Format: 16 digit number (0502605703255600)'); referenceInput.attr('placeholder', '0502605703255600'); break;
            case 'access': hint.text('Format: FJB2606341708208'); referenceInput.attr('placeholder', 'FJB2606341708208'); break;
            case 'absa': hint.text('Format: FJB2606341708208'); referenceInput.attr('placeholder', 'FJB2606341708208'); break;
            case 'withinhere': hint.text('Format: 1777356230718931'); referenceInput.attr('placeholder', '1777356230718931'); break;
            case 'zanaco_online_transfer': hint.text('Format: 002STAJ1234567MN'); referenceInput.attr('placeholder', '002STAJ1234567MN'); break;
            default: hint.text('Enter Payment Reference Number');
        }
    });
    ...
    function validateReferenceFormat(paymentMethod, reference) {
        ...
        switch (paymentMethod) {
            case 'airtel': valid = /^[A-Za-z]{2}\d{6}\.\d{4}\.[A-Za-z]\d{5}$/.test(reference); break;
            case 'airtel_app': valid = /^[A-Za-z]{5}\d{15}$/.test(reference); break;
            case 'zanaco_express': valid = /^\d{12}$/.test(reference); break;
            case 'mtn': valid = /^\d{10}$/.test(reference); break;
            case 'zanaco_cash': valid = /^\d{16}$/.test(reference); break;
            case 'access': valid = /^[A-Za-z]{3}\d{13}$/.test(reference); break;
            case 'absa': valid = /^[A-Za-z]{3}\d{10}$/.test(reference); break;
            case 'withinhere': valid = /^\d+$/.test(reference); break;
            case 'zanaco_online_transfer': valid = /^\d{3}[A-Za-z]{4}\d{7}[A-Za-z]{2}$/.test(reference); break;
        }
        return valid;
    }
    ...
    $(document).on('click', '.complete-btn', function() {
        ...
        let currentDepositAmount = parseFloat(box.find('.amount').val());
        let currentReferenceNumber = box.find('.reference').val().trim();
        let paymentMethod = box.find('.payment-method').val();

        if (!paymentMethod) { KiloAlert.warning('Please select a payment method.'); return; }
        if (!currentReferenceNumber) { KiloAlert.warning('Please enter a payment reference number.'); return; }
        if (isNaN(currentDepositAmount) || currentDepositAmount <= 0) { KiloAlert.warning('Enter a valid amount to add'); return; }
        if (!validateReferenceFormat(paymentMethod, currentReferenceNumber)) { KiloAlert.warning('Invalid reference format for selected payment method.'); return; }

        $btn.prop('disabled', true).find('.btn-text').hide().end().find('.btn-loader').show();
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                box.find('.amount, .reference').val('');
                box.find('.payment-method').val('');
                $btn.prop('disabled', false).find('.btn-text').show().end().find('.btn-loader').hide();
