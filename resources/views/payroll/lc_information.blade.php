...
{{csrf_field()}}
	...
	@endif
	 ...
	 <label for="office_id"
        class="control-label col-md-2">Cycle Dates</label>
        <div class="col-md-3">
        <select name="cycle" class="form-control select2" id="cycle" required>
                            <option></option>
                            @foreach($dates as $date)
                                <option value="{{$target_dates[$date]}}">{{date("jS M Y", strtotime($compare_dates[$date]))}} - {{date("jS M Y", strtotime($target_dates[$date]))}}</option>
                            @endforeach
                        </select>
        </div>
...
@if(!empty($branch))
<div >
<p style="font-weight: bold;">{{\App\Models\Office::where('id',$branch)->first()->name}}  |  {{date("jS M", strtotime($compareDate))}} - {{date("jS M", strtotime($targetDate))}}</p>
</div>
