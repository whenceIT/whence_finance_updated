...
@section('content')
    <div class="box box-primary">
        ...
        <form method="post" action="{{url('asset/type/'.$asset_type->id.'/update')}}" class="form-horizontal"
                    ...
                    <div class="col-md-9">
                        <select name="gl_account_asset_id" class="form-control select2"
                            ...
                            @foreach(\App\Models\GlAccount::where('active',1)->where('account_type',"asset")->get() as $key)
                                <option value="{{$key->id}}"
                                        @if($asset_type->gl_account_asset_id==$key->id) selected @endif>{{$key->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <select name="gl_account_fixed_asset_id" class="form-control select2"
                            ...
                            @foreach(\App\Models\GlAccount::where('active',1)->where('account_type',"asset")->get() as $key)
                                <option value="{{$key->id}}"
                                        @if($asset_type->gl_account_fixed_asset_id==$key->id) selected @endif>{{$key->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-9">
                        <select name="gl_account_expense_id" class="form-control select2"
                            ...
                            @foreach(\App\Models\GlAccount::where('active',1)->where('account_type',"expense")->get() as $key)
                                <option value="{{$key->id}}"
                                        @if($asset_type->gl_account_expense_id==$key->id) selected @endif>{{$key->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <select name="gl_account_contra_asset_id" class="form-control select2"
                            ...
                            @foreach(\App\Models\GlAccount::where('active',1)->where('account_type',"asset")->get() as $key)
                                <option value="{{$key->id}}"
                                        @if($asset_type->gl_account_contra_asset_id==$key->id) selected @endif>{{$key->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-9">
                        <select name="gl_account_income_id" class="form-control select2"
                            ...
                            @foreach(\App\Models\GlAccount::where('active',1)->where('account_type',"income")->get() as $key)
                                <option value="{{$key->id}}"
                                        @if($asset_type->gl_account_income_id==$key->id) selected @endif>{{$key->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <textarea name="notes" class="form-control"
                                  id="notes">{{$asset_type->notes}}</textarea>
                    </div>
                </div>
            </div>
            <!-- /.box-body -->
