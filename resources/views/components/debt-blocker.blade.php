...
@if(!$blocker['status'])
        ...
        <div id="debt-blocker-card" style="background: #ffffff; border-radius: 12px; box-shadow: 0 15px 40px rgba(0,0,0,0.35); max-width: 480px; width: 94%; padding: 28px 24px; text-align: center; border: 2px solid #e74c3c;">
            ...
            <div id="blocker-warning-state">
                ...
                <p style="font-size: 16.5px; color: #444; line-height: 1.55; margin-bottom: 22px;">
                    Please make your <strong style="color:#c0392b; font-size: 18px;">K{{ number_format($blocker['balance'], 0) }}</strong> required monthly deposit<br>
                    to proceed with loan operations.
                </p>
            ...
            <div id="blocker-deposit-form" style="display: none; text-align: left;">
                <h4 style="color: #c0392b; font-weight: 700; margin-bottom: 4px; font-size: 18px;">
                    Record K{{ number_format($blocker['balance'], 0) }} Setup Deposit
                </h4>
                ...
                <br>
                <p>Current Outstanding balance:{{ number_format($blocker['balance'], 0) }}</p>
    ...
    (function () {
        ...
        var currentDepositType = 0; // Setup Debt (matches BlockerHelper)
        var requiredBalance = {{ $blocker['balance'] ?? 0 }};
        ...
        $('#blocker-submit-btn').on('click', function () {
            ...
            if (!confirm('Confirm: The K' + requiredBalance + ' deposit has been made and the details above are correct?')) {
                return;
            }
