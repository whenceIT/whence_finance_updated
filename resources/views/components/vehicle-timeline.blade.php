...
@php
    ...
    $statusLabels = [
        'available' => 'Vehicle is available for sale or use.',
        'in_custody' => 'Vehicle is in our custody, awaiting sale.',
        'sold' => 'Vehicle has been sold and proceeds received.',
    ];
    ...
    <div style="background: linear-gradient(135deg, #f8fafc 0%, #eef2ff 100%); border: 1px solid #e2e8f0; border-radius: 10px; padding: 12px 18px; margin-top: 12px; display: flex; align-items: flex-start; gap: 12px;">
        ...
        <div>
            <div style="font-size: 13.5px; color: #1e293b; line-height: 1.6;">
                <strong>Current Stage: {{ $allStatuses[$currentStep] ?? 'Available' }}</strong> — {{ $statusLabels[$currentStep] ?? $statusLabels['available'] }}
            </div>
            <div style="font-size: 12px; color: #6b7280; margin-top: 4px;">This is the first step in the Vehicle Workflow.</div>
        </div>
    </div>
    @endif
</div>
