            ...
            <form method="post" action="{{Request::url()}}" class="form-horizontal" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="start_date" class="control-label col-md-2">{{trans_choice('general.start', 1)}}
                        {{trans_choice('general.date', 1)}}</label>
                    <div class="col-md-3">
                ...
                <div class="form-group">
                    <label for="end_date" class="control-label col-md-2">{{trans_choice('general.end', 1)}}
                        {{trans_choice('general.date', 1)}}</label>
                    <div class="col-md-3">
                        <input type="text" name="end_date" class="form-control date-picker" value="{{$end_date}}" required
                            id="end_date">
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="btn-group">
                            <button type="button" class="btn bg-blue dropdown-toggle legitRipple"
                                data-toggle="dropdown">{{trans_choice('general.download', 1)}}
                                {{trans_choice('general.report', 1)}}
                                <span class="caret"></span></button>
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li>
                                    <a href="{{url('report/loan_report/expected_repayments/pdf?start_date=' . $start_date . '&end_date=' . $end_date . '&office_id=' . $office_id)}}"
                                        target="_blank"><i class="icon-file-pdf"></i>
                                        {{trans_choice('general.download', 1)}}
                                        {{trans_choice('general.to', 1)}} {{trans_choice('general.pdf', 1)}}
                                    </a>
                                </li>
                                <li>
                                    <a href="{{url('report/loan_report/expected_repayments/excel?start_date=' . $start_date . '&end_date=' . $end_date . '&office_id=' . $office_id)}}"
                                        target="_blank"><i class="icon-file-excel"></i>
                                        {{trans_choice('general.download', 1)}} {{trans_choice('general.to', 1)}}
                                        {{trans_choice('general.excel', 1)}}
                                    </a>
                                </li>
                                <li>
                                    <a href="{{url('report/loan_report/expected_repayments/csv?start_date=' . $start_date . '&end_date=' . $end_date . '&office_id=' . $office_id)}}"
                                        target="_blank"><i class="icon-download"></i>
                                        {{trans_choice('general.download', 1)}}
        ...
        <div class="box box-primary">
                ...
                <table cellspacing="0" cellpadding="0" class="style-0">
                    <tbody>
                        <tr style="height: 25pt">
                            <td colspan="28" valign="middle" class="style-1">{{trans_choice('general.expected', 1)}}
                                {{trans_choice('general.vs', 1)}} {{trans_choice('general.actual', 1)}}
                        ...
                        <tr style="height: 15pt">
                            ...
                            <td colspan="6" valign="middle" class="style-4">{{trans_choice('general.report', 1)}}
                                {{trans_choice('general.run', 1)}} {{trans_choice('general.date', 1)}}
                                :
                            </td>
                        ...
                        <tr style="height: 7pt">
                            <td colspan="4" valign="middle" class="style-6">{{trans_choice('general.cash_flow', 1)}}
                                {{trans_choice('general.per', 1)}} {{trans_choice('general.office', 1)}}
                            </td>
            ...
            $total_actual_repayments = 0;
                            ...
                            $total_actual_repayments = $total_actual_repayments + $actual_repayments;
                                    ...
                                    @if($total_expected_repayments > 0 || $total_actual_repayments > 0)
                                        <tr style="height: 15pt">
                                            ...
                                            <td colspan="3" valign="middle" class="style-14">{{number_format($actual_principal, 2)}}</td>
                                            <td colspan="4" valign="middle" class="style-14">{{number_format($actual_interest, 2)}}</td>
                                            <td colspan="3" valign="middle" class="style-14">{{number_format($actual_fees, 2)}}</td>
                                            <td colspan="3" valign="middle" class="style-14">{{number_format($actual_penalty, 2)}}</td>
                                            <td colspan="2" valign="middle" class="style-14">{{number_format($actual_repayments, 2)}}</td>
                                            <td colspan="2" valign="middle" class="style-13">{{number_format($expected_principal, 2)}}</td>
                                            <td colspan="2" valign="middle" class="style-14">{{number_format($expected_interest, 2)}}</td>
                                            <td colspan="2" valign="middle" class="style-14">{{number_format($expected_fees, 2)}}</td>
                                            <td colspan="2" valign="middle" class="style-14">{{number_format($expected_penalty, 2)}}</td>
                                            <td colspan="2" valign="middle" class="style-14">{{number_format($expected_repayments, 2)}}</td>
                        ...
                        <tr style="height: 17pt">
                            ...
                            <td colspan="2" valign="middle" class="style-17">{{number_format($total_expected_principal, 2)}}
                            </td>
                            ...
                            <td colspan="2" valign="middle" class="style-17">{{number_format($total_expected_repayments, 2)}}
                            </td>
                        </tr>
                        <tr style="height: 0pt">
                            <td colspan="3"></td>
    ...
    <script>
        $(document).ready(function () {
            ...
            function filterLoanOfficers() {
                ...
                $loanOfficerSelect.empty();

                // Filter and append options
                $allLoanOfficerOptions.each(function () {
                    var optionOfficeId = $(this).attr('data-office_id');
                    var optionVal = $(this).val();
                    var $opt = $(this).clone();

                    if (optionVal == "0") {
                         $loanOfficerSelect.append($opt);
                    } else if (officeId == "0") {
                         $loanOfficerSelect.append($opt);
                    } else if (optionOfficeId == officeId) {
                         $loanOfficerSelect.append($opt);
                    }
                });

                // Restore selection if valid, otherwise select "All" (0)
                if ($loanOfficerSelect.find('option[value="' + currentLoanOfficerId + '"]').length > 0) {
                     $loanOfficerSelect.val(currentLoanOfficerId);
                } else {
